...
$("#subscribe-form").submit(event => {
    ...
    const method = {
        ...
        $.ajax(method).done(response => {
            ...
            if (data.status == 200 && data.subscribed == true) {
                alert("The subscriber with this email already exists");
            } else if (data.status == 200 && data.subscribed == false) {
                alert("You subscribed to our website");
            }
        })
})